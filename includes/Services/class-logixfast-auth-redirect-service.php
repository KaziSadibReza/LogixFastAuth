...
class RedirectService {
	...
	private static function get_default_url( $kind, $context ) {
		...
		if ( 'login' === $kind && $context instanceof \WP_User ) {
			$url = apply_filters( 'logixfast_auth_login_redirect', $url, $context ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- LogixFastAuth plugin hook.
		}

		if ( 'register' === $kind ) {
			$user_id = is_int( $context ) ? $context : 0;
			$url     = apply_filters( 'logixfast_auth_register_redirect', $url, $user_id ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- LogixFastAuth plugin hook.
		}
	...
	private static function get_dashboard_default_url( $user ) {
		...
		return (string) apply_filters( 'logixfast_auth_logged_in_login_page_redirect', $url, $user ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- LogixFastAuth plugin hook.
	}
}
