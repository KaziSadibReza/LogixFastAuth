<?php
/**
 * Dedicated LogixFastAuth login page creation and WordPress page labeling.
 *
...
class LoginPageService {
	...
	/**
	 * Mark LogixFastAuth login pages in the WordPress pages list (like WooCommerce).
	 *
	...
	public function add_post_state_label( $post_states, $post ) {
		...
		if ( self::is_login_page( $post->ID ) ) {
			$post_states['logixfast_auth_login_page'] = __( 'LogixFastAuth Sign In Page', 'logixfast-auth' );
		}
	...
	/**
	 * Whether a page is the LogixFastAuth dedicated login page.
	 *
	...
	public static function ensure_dedicated_page() {
		...
		$page_id = wp_insert_post(
			array(
				'post_title'   => __( 'Sign In & Register', 'logixfast-auth' ),
	...
	/**
	 * Tag a page as the LogixFastAuth login page.
	 *
	 * @param int $page_id Page ID.
	 * @return void
	 */
	...
	private static function maybe_upgrade_legacy_page_title( $page_id ) {
		...
		$legacy_titles = array( 'Login', __( 'Login', 'logixfast-auth' ) );
