<?php
/**
 * Bulk-sync user phone fields across SLR, WooCommerce, and Tutor LMS.
 *
 * @package SLR
 */
...
class Phone_Sync_Service {
	...
	public static function sync( $target ) {
		...
		if ( ! in_array( $target, array( self::TARGET_WOOCOMMERCE, self::TARGET_TUTOR, self::TARGET_ALL ), true ) ) {
			return new \WP_Error( 'slr_phone_sync_invalid', __( 'Invalid phone sync target.', 'smart-login-registration' ), array( 'status' => 400 ) );
		}

		$meta_keys = self::get_target_meta_keys( $target );
		if ( empty( $meta_keys ) ) {
			return new \WP_Error( 'slr_phone_sync_unavailable', __( 'No phone sync targets are available.', 'smart-login-registration' ), array( 'status' => 400 ) );
		}
	...
	/**
	 * Prefer SLR legacy data, then WooCommerce, then Tutor.
	 *
