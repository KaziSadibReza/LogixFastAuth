...
class Activator {
	...
	public static function maybe_upgrade() {
		...
		$pending_table  = $wpdb->prefix . 'logixfast_auth_pending_registrations';
		$otp_table      = $wpdb->prefix . 'logixfast_auth_otp_codes';
		$webauthn_table = $wpdb->prefix . 'logixfast_auth_webauthn_credentials';
		...
		if ( $has_pending && $has_otp && $has_webauthn && get_option( 'logixfast_auth_db_version' ) === LOGIXFAST_AUTH_VERSION ) {
			return;
		}

		self::create_tables();
		update_option( 'logixfast_auth_db_version', LOGIXFAST_AUTH_VERSION );
	}

	/**
	 * Ensure all required LogixFastAuth tables exist. Safe to call on every request;
