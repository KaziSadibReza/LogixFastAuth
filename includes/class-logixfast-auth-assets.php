...
class Assets {
	...
	public static function is_dev_mode() {
		if ( ! defined( 'LOGIXFAST_AUTH_DEV' ) || ! constant( 'LOGIXFAST_AUTH_DEV' ) ) {
			return false;
		}
	...
	public static function get_dev_server_url() {
		if ( defined( 'LOGIXFAST_AUTH_DEV_URL' ) ) {
			return rtrim( (string) constant( 'LOGIXFAST_AUTH_DEV_URL' ), '/' );
		}

		return rtrim( (string) apply_filters( 'logixfast_auth_dev_server_url', '' ), '/' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- LogixFastAuth plugin hook.
	}
	...
	public static function get_dist_base_url() {
		...
		return LOGIXFAST_AUTH_PLUGIN_URL . 'assets/dist/';
	}
	...
	public static function get_manifest_path() {
		$paths = array(
			LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/manifest.json',
			LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/.vite/manifest.json',
		);
	...
	/**
	 * Localize LOGIXFAST_AUTH_CONFIG for a script handle (used with Vite dev entries).
	 *
	 * @param string $handle Script handle.
	 * @return void
	 */
	public static function enqueue_frontend_config( $handle = 'logixfast-auth-config' ) {
		wp_register_script( $handle, false, array(), LOGIXFAST_AUTH_VERSION, true );
		wp_enqueue_script( $handle );
		wp_localize_script( $handle, 'LOGIXFAST_AUTH_CONFIG', \LogixFastAuth\Settings::get_public_config() );
	}
...
await import(<?php echo wp_json_encode( $entry_url ); ?>);
	...
	public static function enqueue_classic_script( $handle, $relative_path, $deps = array(), $in_footer = true ) {
		$path = LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/' . ltrim( $relative_path, '/' );
		$version = file_exists( $path ) ? (string) filemtime( $path ) : LOGIXFAST_AUTH_VERSION;
	...
	public static function get_asset_url( $path ) {
		$path = ltrim( $path, '/' );
		$full_path = LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/' . $path;
		...
		if ( self::is_dev_mode() && file_exists( $full_path ) ) {
			return LOGIXFAST_AUTH_PLUGIN_URL . 'assets/dist/' . $path;
		}
	...
	public static function get_entry_version( $entry_key, $fallback_path ) {
		...
		if ( $entry && ! empty( $entry['file'] ) ) {
			$path = LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/' . ltrim( $entry['file'], '/' );
			if ( file_exists( $path ) ) {
		...
		if ( $fallback_path ) {
			$path = LOGIXFAST_AUTH_PLUGIN_DIR . 'assets/dist/' . ltrim( $fallback_path, '/' );
			if ( file_exists( $path ) ) {
				return (string) filemtime( $path );
			}
		}

		return LOGIXFAST_AUTH_VERSION;
	}
}
