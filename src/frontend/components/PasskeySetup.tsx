...
export function PasskeySetup({ config, onDone, onSkip }: PasskeySetupProps) {
  ...
  return (
    <div className="logixfast-auth-passkey-setup">
      <div className="logixfast-auth-passkey-setup__icon">
        <Fingerprint size={32} strokeWidth={1.5} />
      </div>

      <h2 className="logixfast-auth-passkey-setup__title">
        {config.i18n.passkeySetupTitle || 'Set up a passkey'}
      </h2>

      <p className="logixfast-auth-passkey-setup__desc">
        {config.i18n.passkeySetupDesc || 'Sign in faster next time with Face ID, Touch ID, or your device PIN. No password needed.'}
      </p>

      <button
        type="button"
        className="logixfast-auth-btn logixfast-auth-btn--primary"
        onClick={handleSetup}
        disabled={loading}
      >
        {loading ? (
