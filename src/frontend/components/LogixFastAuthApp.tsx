...
import type { LogixFastAuthConfig, LogixFastAuthMode } from '@shared/types';
...
import { LogixFastAuthTabSwitch } from './LogixFastAuthTabSwitch';
import { LogixFastAuthToastProvider } from './LogixFastAuthToaster';
import { applyAuthRedirect, type AuthRedirectResult } from '../utils/redirect';
import { consumePendingOpen, markPopupReady } from '../logixfast-auth-bridge';
import {
  clearOtpSession,
  getActiveOtpSession,
  ...
  saveOtpSession,
  type OtpPurpose,
  type LogixFastAuthOtpSession,
} from '../utils/formStorage';

interface LogixFastAuthAppProps {
  config: LogixFastAuthConfig;
  initialOpen?: boolean;
  initialMode?: LogixFastAuthMode;
}

type View = 'form' | 'otp' | 'forgot' | 'reset' | 'passkey-setup';

const PASSKEY_DISMISSED_KEY = 'logixfast_auth_passkey_dismissed';
...
export function LogixFastAuthApp({ config, initialOpen = false, initialMode }: LogixFastAuthAppProps) {
  const saved = loadFormSession();
  const activeOtp = getActiveOtpSession();

  const [open, setOpen] = useState(initialOpen);
  const [mode, setMode] = useState<LogixFastAuthMode>(activeOtp?.mode || initialMode || config.defaultMode || 'login');
  const [view, setView] = useState<View>(activeOtp ? 'otp' : 'form');
  const [otpState, setOtpState] = useState<LogixFastAuthOtpSession | null>(activeOtp);
  ...
  const persistOtp = useCallback((session: LogixFastAuthOtpSession | null) => {
    setOtpState(session);
  ...
  const maybeOfferPasskey = useCallback(
    async (result: AuthRedirectResult, kind: 'login' | 'register') => {
      ...
      if (result.nonce && window.LOGIXFAST_AUTH_CONFIG) {
        window.LOGIXFAST_AUTH_CONFIG.nonce = result.nonce;
      }
  ...
  const resumeOtp = useCallback((session: LogixFastAuthOtpSession) => {
    setOtpState(session);
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      const detail = (e as CustomEvent).detail as { mode?: LogixFastAuthMode };
    ...
    window.addEventListener('logixfastauth:open', onOpen);
    window.addEventListener('logixfastauth:close', onClose);
    return () => {
      window.removeEventListener('logixfastauth:open', onOpen);
      window.removeEventListener('logixfastauth:close', onClose);
    };
  ...
  const overlayClass = [
    'logixfast-auth-overlay',
    'logixfast-auth-overlay--glass',
    config.isDedicated ? 'logixfast-auth-overlay--page' : '',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  return (
    <LogixFastAuthToastProvider>
    <div
      ref={overlayRef}
      className={overlayClass}
      role="dialog"
      aria-modal="true"
      aria-labelledby="logixfast-auth-title"
      aria-hidden={!open && !config.isDedicated ? 'true' : 'false'}
      onClick={(e) => { if (e.target === overlayRef.current && !config.isDedicated) handleClose(); }}
    >
      <div ref={cardRef} className="logixfast-auth-card">
        <button type="button" className="logixfast-auth-close" onClick={handleClose} aria-label={config.i18n.close}>
          <X size={20} strokeWidth={2} aria-hidden="true" />
        </button>

        {view === 'form' && (
          <>
            <h2 id="logixfast-auth-title" className="logixfast-auth-title">
              {mode === 'login' ? (config.i18n.loginTitle || 'Welcome back') : (config.i18n.registerTitle || 'Create your account')}
            </h2>
            <p className="logixfast-auth-subtitle">
              {mode === 'login'
            ...
            {pendingOtp && (
              <div className="logixfast-auth-otp-resume">
                ...
                <button
                  type="button"
                  className="logixfast-auth-btn logixfast-auth-btn--ghost logixfast-auth-btn--sm"
            ...
            <div key={mode} className="logixfast-auth-form-panel">
              {mode === 'login' ? (
                <LoginForm
